...
This module turns the lower-level ingest/summarization utilities into a
document-centric API: ingest once, then search, compare, cite, visualize, and
generate NotebookLM-style learning artifacts from the same grounded source.
"""
...
DEFAULT_ANALYSIS_ALGORITHMS = ["textrank", "lexrank", "lsa", "vit5", "bartpho"]
VI_STOPWORDS = {
...
class DocumentIntelligenceService:
    ...
    def semantic_search(self, document_id: str, query: str, top_k: int = 5) -> dict[str, Any]:
        ...
        if embeddings:
            ...
            if query_vector is not None and query_vector.shape[0] == vectors.shape[1]:
                scores = self._cosine_scores(query_vector, vectors)
                results = self._ranked_chunk_results(chunks, scores, top_k)
            else:
    ...
    def compare_summaries(
        ...
        *,
        ...
        for row in compare.get("results", []):
            ...
            row["consistency"] = check_consistency(row.get("summary", ""), source_text, mode="fast")
    ...
    def build_visualization(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "embedding_map": self._embedding_map(chunks, embeddings),
            "similarity_heatmap": self._similarity_heatmap(embeddings),
