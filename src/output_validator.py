...
def _is_vietnamese_latin_letter(ch: str) -> bool:
    """True for ASCII Latin and Vietnamese diacritic letters (Unicode Latin blocks)."""
    if not ch.isalpha():
        return False
    try:
...
def foreign_script_ratio(text: str) -> float:
    """
    Share of alphabetic characters that are not Latin/Vietnamese (e.g. Cyrillic, Sinhala).
    Returns 0.0 when there are no letters.
    """
...
def vietnamese_letter_ratio(text: str) -> float:
    """Share of letters that carry Vietnamese diacritics or đ/Đ."""
    letters = [ch for ch in text if ch.isalpha()]
    if not letters:
        return 0.0
...
def is_garbled_abstractive(text: str) -> bool:
    """Detect ViT5/mT5-style corrupted Vietnamese (not just foreign scripts)."""
...
def is_multilingual_garbage(text: str, *, require_vietnamese: bool = False) -> bool:
    """
    Detect short multilingual junk typical of unfine-tuned mT5 (mixed scripts, no Vietnamese).
    """
    ...
    if require_vietnamese:
        ...
        # Vietnamese news summaries usually contain tone marks; pure ASCII Latin is suspicious.
...
def validate_output(text: str, *, require_vietnamese: bool = False) -> dict:
    ...
    # 3. Check for malformed Vietnamese space or unicode errors
