...
def compute_length_targets(
    ...
    *,
    ...
    """
    Derive sentence_count and max_output_length from a target compression ratio.

    target_length_ratio: 10–100 (% of source word count for the summary budget).
    """
...
def trim_summary_to_word_budget(summary: str, target_words: int) -> str:
    """Trim generated summary to the word budget (sentence-aware when possible)."""
...
def words_to_max_new_tokens(word_budget: int) -> int:
    """Map a word budget to a safe HuggingFace max_new_tokens ceiling."""
    words = max(1, int(word_budget))
    return max(24, min(512, int(words * 1.45)))


def min_new_tokens_for_budget(word_budget: int) -> int:
    """Encourage summaries that are not far below the requested word budget."""
    words = max(1, int(word_budget))
...
def allocate_chunk_word_budgets(chunks: list[str], total_word_budget: int) -> list[int]:
    """Split a word budget across chunks proportional to chunk size."""
